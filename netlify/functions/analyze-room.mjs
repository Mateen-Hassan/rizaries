...
export default async (request) => {
  ...
  try {
    ...
    if (!/^image\/(jpeg|png|webp)$/i.test(mimeType)) {
      return jsonResponse(
        { error: 'Please send a JPEG, PNG or WEBP image.' },
        400,
        request
      );
    }
    ...
    const prompt = `
You are Rizaries' visual interior shopping assistant.

Analyze this room photo and return ONLY valid JSON.

Focus on visible room context, not tiny details.
Do not invent exact products.
...
- recommended_collection_handles:
  rank up to 4 exact handles that best fit the whole room,
  considering colors + style + furniture.

- harmony_recommendation:
  pick EXACTLY 2 color-harmony types from this fixed list,
  choosing whichever 2 genuinely suit this room's specific
  style, mood and furniture — not a generic default:
...
- harmony_reasoning:
  an object with those same 2 harmony type names as keys.
  Each value must be one short sentence, maximum 18 words,
  explaining why it fits THIS room specifically,
  referencing something you actually see.
...
`;
    ...
    if (!response.ok) {
      ...
      console.error(
        'Gemini error:',
        raw?.error?.message || response.status
      );
    ...
    if (!text) {
      console.error('Gemini returned empty response:', raw);
    ...
    const allowedHandles = new Set(
      THEMES.map(t => t.handle)
    );

    result.recommended_collection_handles =
      Array.isArray(result.recommended_collection_handles)
        ? result.recommended_collection_handles
            .map(h => String(h).toLowerCase())
            .filter(h => allowedHandles.has(h))
            .slice(0, 4)
        : [];

    // Validate furniture
    result.furniture =
      Array.isArray(result.furniture)
        ? result.furniture
            .map(item => String(item))
            .slice(0, 5)
        : [];

    // Validate harmony types
    const allowedHarmonies = new Set([
      'Complementary',
      'Analogous',
      'Triadic',
      'Monochromatic',
    ...
    result.harmony_recommendation =
      Array.isArray(result.harmony_recommendation)
        ? result.harmony_recommendation
            .filter(h => allowedHarmonies.has(String(h)))
            .slice(0, 2)
        : [];

    // Validate harmony reasoning
    const reasoningIn =
      result.harmony_reasoning &&
      typeof result.harmony_reasoning === 'object'
        ? result.harmony_reasoning
        : {};
    ...
    for (const type of result.harmony_recommendation) {
      ...
      if (typeof reasoningIn[type] === 'string') {
        ...
        result.harmony_reasoning[type] =
          reasoningIn[type].slice(0, 160);
