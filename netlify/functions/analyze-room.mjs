// Netlify Function + Groq Vision.
// Keep GROQ_API_KEY server-side. Never put it in Shopify HTML/browser JS.

const MODEL = process.env.GROQ_MODEL || 'qwen/qwen3.6-27b';
const MAX_BASE64_LENGTH = 3_600_000; // Keep comfortably below Groq's 4 MB base64 request limit.
...
export default async (request) => {
  ...
  if (!process.env.GROQ_API_KEY) return jsonResponse({ error: 'GROQ_API_KEY is not configured' }, 500, request);
...
{"room_type":"","style":"","mood":"","dominant_color":"","secondary_color":"","floor_color":"","furniture":[],"recommended_collection_handles":[],"harmony_recommendation":[],"harmony_reasoning":{}}`;
    ...
    if (!response.ok) {
      console.error('Groq error:', raw?.error?.message || response.status);
